// ── Region Manager Notifications JS — API-backed ──
...
(async () => {
  ...
  // Fetch data from API
  let allProviders = [], allBookings = [], allTransactions = [], allRegions = [], allUnits = [], allJobAssignments = [], allProviderSkills = [], allSkills = [];
  allProviders  = await Api.get("/service-providers");
  allBookings  = await Api.get("/bookings");
  allTransactions  = await Api.get("/transactions");
  allRegions  = await Api.get("/regions");
  ...
  // Mapping collective -> region_ids
  const myRegion = allRegions.find(
    (c) => c.region_id === collectiveId,
  );
  const myRegionId = myRegion ? myRegion.region_id : null;

  // Find providers in CM's sectors that don't belong to a unit yet
  const myUnassignedProviders = allProviders.filter(
    (p) => !p.region_id && mySectors.includes(p.city),
  );
  ...
  if (myUnassignedProviders.length > 0) {
    allNotifications.push({
      ...
      desc: `${myUnassignedProviders.length} new provider application(s) pending approval for your region.`,
      time: "Recently",
  ...
  pendingSkills.forEach((ps) => {
    ...
    if (!myProviderIds.has(provider.region_id)) return;
