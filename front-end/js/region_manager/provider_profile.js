...
(async () => {
  ...
  let allUnits = [], allRegions = [], allSectors = [], allProviderSkills = [], allSkills = [], allJobs = [];
  ...
  const unit = allUnits.find(u => u.region_id === provider.region_id);
  const collective = unit ? allRegions.find(c => c.region_id === unit.region_id) : null;
  const sector = regions.find(s => s.region_id === provider.city);
  ...
  document.getElementById('infoSector').textContent = sector ? region ? region.region_name : provider.city || "Chennai" : 'N/A';
