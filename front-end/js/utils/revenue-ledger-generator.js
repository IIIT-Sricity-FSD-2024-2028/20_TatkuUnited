 ...
 * Revenue split: Provider 85%, Super User / Platform 15%
 */
...
const RevenueManager = {
  ...
  ensureLedgerEntriesForBooking: async function (bookingId, options) {
    ...
    const providerId = (jobAssignment && (jobAssignment.sp_id || jobAssignment.service_provider_id)) || opts.providerId;
    if (!providerId) { console.warn(`[RevenueManager] No provider found for booking: ${bookingId}`); return false; }

    const provider = data.service_providers?.find((sp) => sp.sp_id === providerId || sp.service_provider_id === providerId);
    ...
    const entries = [
      { role: "provider", service_provider_id: provider.sp_id || provider.service_provider_id, amount: Math.round(totalAmount * this.SPLITS.provider * 100) / 100, percentage: 85 },
      { role: "super_user", super_user_id: superUser.super_user_id, amount: Math.round(totalAmount * this.SPLITS.super_user * 100) / 100, percentage: 15 },
    ];
    ...
    console.log(`[RevenueManager] ✅ Created 2 ${payoutStatus} ledger entries for booking ${bookingId}`);
    return true;
  },
