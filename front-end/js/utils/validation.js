...
window.Validators = (() => {
    ...
    function _fail(error) { return { valid: false, error }; }
    ...
    function validateLogin({ email, password } = {}) {
        ...
        if (!_isValidEmail(email)) return _fail("Please enter a valid email address.");
    ...
    function validateScheduledAt(scheduledAt) {
        ...
        if (selected <= now) return _fail("Scheduled time must be in the future.");
        if (selected > sevenDaysLater) return _fail("Scheduled time must be within the next 7 days.");
        return _ok();
    }
    ...
    function validateProvider({
        ...
        if (!_isValidPhone(phone))
            return _fail("Phone must be a 10-digit number.");

        if (!_isValidEmail(email))
            return _fail("Please enter a valid email address.");

        if (!(dob || "").trim())
            return _fail("Date of birth is required.");

        const dobDate = new Date(dob);
        if (isNaN(dobDate))
            return _fail("Invalid date of birth.");
        if (dobDate >= new Date())
        ...
        if (age < 18)
            return _fail("Provider must be at least 18 years old.");
    ...
    function validateService({
        ...
        if (!(service_name || "").trim())
            return _fail("Service name is required.");
        ...
        if (isNaN(duration) || duration <= 0 || !Number.isInteger(duration))
            return _fail("Estimated duration must be a positive whole number (minutes).");
    ...
    function validateUnit({ unit_name, collective_id } = {}) {
        if (!(unit_name || "").trim())
            return _fail("Unit name is required.");
